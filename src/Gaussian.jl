...
include("Basis.jl")
using .Basis: AbstractFunctionSet
...
nicesigma(μ::AbstractMatrix, d=1.5) = nicesigma.((view(μ, :, i) for i=1:size(μ, 2)), d)
nicesigma(μ::AbstractVector, d=1.5) = nicesigma(minimum(μ), maximum(μ), size(μ, 1), d)
nicesigma(μmin, μmax, N, d=1.5) = nicesigma((μmax - μmin)/(N - 1), d)
"Δ: distance between two gaussian peak. Δ*d: distance for half value of the guaussian"
function nicesigma(Δ::Real, d=1.5)
    Δ*d/sqrt(log(4))
end
...
function GaussianSet(xmin, xmax, xn, Δ=1.5)
